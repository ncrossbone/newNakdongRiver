function wuSewerView() {
	...
	function setSearch() {
		...
		$search_type_mw.ready(function() {
			drawComboBox('/nakdong/wu/common_mw.json', $search_type_mw, '중권역');
		});
		
		// 시도 리스트 
		$search_type_sido.ready(function() {
			drawComboBox('/nakdong/wu/common_sido.json', $search_type_sido, '시도');
		});
		...
		$search_type_sido.bind({
			change: function() {
				...
				drawComboBox('/nakdong/wu/common_sgg.json?sido=' + sido, $search_type_sgg, '시군구');
			}
		});
		...
		$search_btn_search.bind({
			click: function() {
				...
				var sidot = $search_type_sido.find("option:selected").text();
				var sggv = $search_type_sgg.find("option:selected").val();
				var sggt = $search_type_sgg.find("option:selected").text();
				...
				if(groupv == 1) {
					// 중권역 이동
					pageMap.moveForSearch('중권역', 12, mwv);
				} else if(groupv == 2) {
					// 시군구 이동
					if(sggv != "none") {
						pageMap.moveForSearch('시군구', 25, sggv);
					} 
					
					// 시도 이동
					else {
						pageMap.moveForSearch('시도', 27, sidov);
					}
				}
				
				// 차트, 테이블 관련 결과
				if (groupv == 1) {
					popup("/nakdong/chartWuSewer1.do?mw=" + mwv, 852, 302, false);
				} else if(groupv == 2) {
					popup("/nakdong/chartWuSewer2.do?sgg=" + encodeURIComponent(encodeURIComponent(sggt, "UTF-8")) + "&sido=" + encodeURIComponent(encodeURIComponent(sidot, "UTF-8")), 852, 302, false);
				}
			}
		});
