function plLoadView() {
	...
	// 검색 설정
	function setSearch() {
		var $search_type_type = $("#search_type_type");//오염원
		var $search_type_year = $("#search_type_year");
		var $search_type_mw = $("#search_type_mw");
		var $search_type_buha = $("#search_type_buha");//부하량
		...
		$search_type_mw.ready(function() {
			drawComboBox('/nakdong/pl/common_mw.json', $search_type_mw, '중권역');
		});
		...
		$search_btn_search.bind({
			click: function() {
				...
				var year = $search_type_year.find("option:selected").val();
				var mw = $search_type_mw.find("option:selected").val();
				var buha = $search_type_buha.find("option:selected").val();
				// 검색 조건 체크
				if(type == "none") {
...
//				
//				alert(layer.url + "/" + type);
//				setLegend(layer.url + "/" + type);
				
				// 차트, 테이블 관련 결과 2013-10-15수정
				popup("/nakdong//chartPlLoad.do?search_type_type="+type+"&search_type_year="+year+"&search_type_mw="+mw+"&search_type_buha="+buha, 617, 617, false);
			}
		});
...
//				layer.setVisibleLayers([]);
				...
				$search_type_buha.find("option:first").attr("selected", "selected");
...
//				$search_type_umd.find("option:first").attr("selected", "selected");
