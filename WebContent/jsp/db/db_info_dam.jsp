...
<head>
	...
	<script>
		function getList() {
			...
			$.ajax({
				url: '/nakdong/db/common_dam.json?year=' + year + '&month=' + month + '&day=' + day + '&hour=' + hour
				, dataType: 'json'
				, success: function(d) {
					var result = d.result;
					...
					for(var i in result) {
						var html = "<tr onmouseover=\"this.bgColor='#ffcc00';this.style.cursor='pointer';\" onmouseout=\"this.bgColor='#ffffff';\" id=\"" + result[i].DMOBSCD + "\" bgcolor=\"#ffffff\" style=\"cursor: pointer;\">" +
							"    <td>" + result[i].OBSNM + "</td>" +
							"    <td class=\"value\">" + setComma(parseFloat(result[i].PFH).toFixed(2)) + "</td>" +
					...
					$tbody.find("tr").bind("click", function() {
						...
						$div.append("<object type='text/html' data='/nakdong/resources/js/chart/db/dam/app.html?damcode=" + this.id + "&year=" + $("#year option:selected").val() + "' id='infoBoChart'></object>");
					});
				}
			});
		}
...
<body>
	...
	<div class="tableDiv">
		<table border="0" cellpadding="0" cellspacing="0" id="infoDam">
			<thead>
		        <tr>
		            <th class="head">지점</th>
		            <th class="head">계획홍수위(m)</th>
		            <th class="head">저수위(m)</th>
	...
	<div class="infoDamChartDiv" id="infoDamChartDiv">
	</div>
	
	<div class="infoDamClose">
